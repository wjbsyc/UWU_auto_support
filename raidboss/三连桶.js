...
function NamazuHTTPMark(actorID, markType){
  //HTTPpostNamazu('mark', `{ \"ActorID\": ${int_id}, \"MarkType\": \"attack${nails[j].mark}\" }`);
  try{
    HTTPpostNamazu('mark', `{ \"ActorID\": ${actorID}, \"MarkType\": \"${markType}\" }`);
  }catch{}
}
    ...
    },
    {
      ...
      infoText: (data, _matches, output) => {
        if ((output.enable() === "true" || output.enable() === "开") && data.titanGaolsMark?.length === 3) {
          try{
            NamazuHTTPMark(parseInt(data.titanGaolsMark[0].id, 16), "attack1");
            NamazuHTTPMark(parseInt(data.titanGaolsMark[1].id, 16), "attack2");
            NamazuHTTPMark(parseInt(data.titanGaolsMark[2].id, 16), "attack3");
            HTTPpostNamazu('command', `/p 1:${data.ShortName(data.titanGaolsMark[0].name)}`); 
            HTTPpostNamazu('command', `/p 2:${data.ShortName(data.titanGaolsMark[1].name)}`); 
            HTTPpostNamazu('command', `/p 3:${data.ShortName(data.titanGaolsMark[2].name)}`); 
          }catch {};
          return output.text({
